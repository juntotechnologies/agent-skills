...
- Local verification is the proof. Before reporting work as done, run typecheck, lint, unit tests, and E2E locally on the exact commit being reported, and state the results. Never report results from an earlier session or another agent as if you had run them - rerun, or say they weren't rerun.
- Don't dispatch or check GitHub CI routinely; a passing local run on the same commit already covers it. Run CI only when a change touches the CI config/workflows themselves, or when I ask (e.g. right before merge). When you do run it, wait for it to finish before reporting.
